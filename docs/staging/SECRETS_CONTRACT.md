...
| Vercel project (planned) | `settlerate-app-staging` (separate from production) |
...
| `STRIPE_SECRET_KEY` | **Yes** | `sk_test_…` only |
| `STRIPE_WEBHOOK_SECRET` | **Yes** | Test-mode endpoint secret only |
| `CHECKOUT_MAINTENANCE` | No | May be `false` on staging for test checkout |
| `SENTRY_DSN` | No (public DSN) | Staging Edge project/DSN; environment `staging` |
