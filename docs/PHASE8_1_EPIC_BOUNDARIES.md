...
**Status:** **In progress — PR 1.** ADR 0005 accepted; PR 0 complete/merged.
PR 1 scaffolds npm workspaces and `packages/core` (`@settlerate/core`) with
**no business logic migrated** and **no app/Edge runtime consumers**.
PR 2–6 (module extraction) remain unauthorized. Epic 6+ remain unauthorized.
...
- Migrating business modules or wiring app/Edge runtime consumers in PR 1
  (scaffold only)
...
| **PR 1** | `packages/core` workspace scaffold (no behavioral migration) | **In progress** (this slice) |
...
**Epic 5 is in progress (PR 1 — scaffold only). Do not begin PR 2–6 or
Epic 6+ automatically.**
